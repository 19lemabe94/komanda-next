...
export default function ProductsPage() {
  ...
  const cancelEditCategory = () => { setEditingCategory(null); setCatName(''); setCatColor(COLOR_PALETTE[10]) }

  const handleSaveCategory = async (e: FormEvent) => {
    e.preventDefault()
    if (!catName.trim() || !myOrgId) return
    if (editingCategory) {
      await supabase.from('categories').update({ name: catName.trim(), color: catColor }).eq('id', editingCategory.id)
      cancelEditCategory()
    } else {
      await supabase.from('categories').insert([{ name: catName.trim(), color: catColor, org_id: myOrgId }])
      setCatName('')
    }
    fetchData(myOrgId)
  }
  ...
  return (
    <div style={{ ...globalStyles.container, justifyContent: 'flex-start', background: '#f8fafc' }}>
      ...
      <main style={{ width: '100%', maxWidth: '900px', padding: '20px', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ position: 'relative', marginBottom: '15px' }}>
          ...
          <input type="text" placeholder="Buscar produto..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
            style={{ width: '100%', padding: '15px 15px 15px 50px', borderRadius: '12px', border: `1px solid ${colors.border}`, fontSize: '1rem', outline: 'none', height: '52px' }} />
          {searchTerm && <button onClick={() => setSearchTerm('')} style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', border: 'none', background: 'none', cursor: 'pointer', color: '#94a3b8' }}><IconX /></button>}
        </div>
          ...
          <div style={{ ...globalStyles.card, width: '100%', maxWidth: '480px', padding: '30px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleSaveProduct}>
              ...
              <div style={{ marginBottom: '20px', textAlign: 'center' }}>
                {prodForm.image_url ? (
                  <div style={{ position: 'relative', display: 'inline-block' }}>
                    <img src={prodForm.image_url} alt="Preview" style={{ width: '120px', height: '120px', objectFit: 'cover', borderRadius: '16px', border: `2px solid ${colors.border}` }} />
                    <button type="button" onClick={() => setProdForm(f => ({ ...f, image_url: '' }))}
                      style={{ position: 'absolute', top: '-8px', right: '-8px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '50%', width: '24px', height: '24px', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 900 }}>✕</button>
                  </div>
                ) : (
                  <label style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', width: '120px', height: '120px', border: `2px dashed ${colors.border}`, borderRadius: '16px', cursor: 'pointer', color: colors.textMuted, background: '#f8fafc', gap: '8px' }}>
                    <span style={{ fontSize: '2rem' }}>📷</span>
                    <span style={{ fontSize: '0.75rem', fontWeight: 700 }}>{imageUploading ? 'Enviando...' : 'Adicionar foto'}</span>
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '20px' }}>
                ...
                <button type="submit" disabled={submitting || imageUploading} style={{ ...touchBtnStyle, backgroundColor: grenaColor, color: 'white', flex: 1 }}>
                  {submitting ? 'Salvando...' : imageUploading ? 'Aguarde foto...' : 'Salvar'}
                </button>
              </div>
            </form>
          </div>
          ...
          <div style={{ ...globalStyles.card, width: '90%', maxWidth: '380px', padding: '30px' }}>
            ...
            <div style={{ borderTop: `1px solid ${colors.border}`, paddingTop: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                ...
                {editingCategory && <button onClick={cancelEditCategory} style={{ fontSize: '0.8rem', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Cancelar</button>}
              </div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {COLOR_PALETTE.map(color => (
                  <button key={color} type="button" onClick={() => setCatColor(color)}
                    style={{ width: '32px', height: '32px', borderRadius: '50%', backgroundColor: color, cursor: 'pointer', border: catColor === color ? '3px solid #0f172a' : '2px solid transparent', transform: catColor === color ? 'scale(1.1)' : 'scale(1)', transition: 'all 0.2s' }} />
                ))}
              </div>
            </div>
          </div>
      ...
      <MenuExportModal isOpen={isExportModalOpen} onClose={() => setIsExportModalOpen(false)} categories={categories} products={products} />
