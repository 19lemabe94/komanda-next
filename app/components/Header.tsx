...
export function Header({ userRole, subtitle = 'OPERAÇÃO' }: HeaderProps) {
  ...
  // --- ESTILOS MOBILE (Botões Maiores) ---
  const mobileBtnStyle = {
    ...btnBaseStyle,
    width: '100%',
    justifyContent: 'flex-start',
    padding: '18px 20px', // Mais espaçamento para toque
    fontSize: '1.1rem',
    border: `1px solid #f1f5f9`,
    background: 'white',
    color: colors.text,
    marginBottom: '8px' // Separação entre botões
  }
  ...
  // OVERLAY DO MENU MOBILE (Ocupa 85% da tela, sem bordas arredondadas estranhas)
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 90, backdropFilter: 'blur(4px)' }} onClick={() => setIsMobileMenuOpen(false)}>
      <div 
        ...
        style={{ 
            ...
            width: '85%', // Ocupa 85% da largura do celular (melhor que pixels fixos)
            maxWidth: '400px', // Trava em tablets
            background: 'white', 
            padding: '25px', 
            display: 'flex', 
            flexDirection: 'column', 
            boxShadow: '-5px 0 25px rgba(0,0,0,0.2)',
            borderTopLeftRadius: '20px', // Apenas canto esquerdo arredondado
            borderBottomLeftRadius: '20px',
            overflowY: 'auto' // Permite rolar se a tela for pequena
        }}
      >
        ...
        <div style={{ marginBottom: '30px', paddingBottom: '20px', borderBottom: `2px solid ${colors.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
           ...
           <button 
             ...
             style={{ background: '#f8fafc', border: 'none', borderRadius: '50%', width: '45px', height: '45px', fontSize: '1.4rem', color: colors.text, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
           >
             ✕
           </button>
        </div>
        ...
        <div style={{ marginTop: '20px', borderTop: `1px solid ${colors.border}`, paddingTop: '20px' }}>
            <button onClick={async () => { await supabase.auth.signOut(); router.push('/login') }} style={{ ...mobileBtnStyle, backgroundColor: '#fee2e2', color: '#ef4444', border: 'none', justifyContent: 'center' }}>
