...
export function OrderDetailsModal({ orderId, label, onPayment, onClose, onUpdate, userRole }: Props) {
  ...
  const loadData = async () => {
    try {
      ...
      if (profile?.org_id) {
        ...
        const [itemsRes, prodRes, catRes, payRes, clientRes] = await Promise.all([
  ...
  const processPayment = async (method: string, clientId: string | null) => {
    ...
    if (onPayment) {
        await onPayment(orderId, val, method, clientId)
        onClose()
    }
  }
    ...
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(4px)' }}>
      ...
      <div className="slide-up" style={{ backgroundColor: '#fff', width: '100%', maxWidth: '550px', height: '94vh', borderTopLeftRadius: '24px', borderTopRightRadius: '24px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 25px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${borderLight}` }}>
          <div>
            <h2 style={{ margin: 0, fontSize: '1.3rem', color: luxuryBlack, fontWeight: 700, textTransform: 'capitalize' }}>{label}</h2>
            <p style={{ margin: '2px 0 0', fontSize: '0.85rem', color: textMuted }}>Total: R$ {itemsTotal.toFixed(2)}</p>
          </div>
        ...
        <div style={{ display: 'flex', background: '#fafafa', borderBottom: `1px solid ${borderLight}` }}>
          ...
          <button onClick={() => setActiveTab('comanda')} style={{ flex: 1, padding: '16px', border: 'none', borderBottom: activeTab === 'comanda' ? `2px solid ${luxuryBlack}` : 'none', background: 'none', fontWeight: activeTab === 'comanda' ? 700 : 500, color: activeTab === 'comanda' ? luxuryBlack : textMuted, cursor: 'pointer' }}>Comanda ({items.length})</button>
        </div>
        ...
        {activeTab === 'menu' && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                <div style={{ flex: 1, position: 'relative' }}>
                  <input 
                    ...
                    style={{ width: '100%', padding: '12px 15px 12px 45px', borderRadius: '10px', border: `1px solid ${borderLight}`, outline: 'none', background: isCustomMode ? '#f5f5f5' : '#fff', fontSize: '0.95rem', color: luxuryBlack }} 
                  />
                  <div style={{ position: 'absolute', left: '15px', top: '50%', transform: 'translateY(-50%)', color: '#999', display: 'flex', alignItems: 'center' }}><IconSearch /></div>
                </div>
              </div>
            ...
            <div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              {isCustomMode ? (
                <div className="slide-up" style={{ display: 'flex', flexDirection: 'column', gap: '15px', paddingTop: '10px' }}>
                  <div style={{ padding: '15px', background: '#fff1f2', borderRadius: '12px', border: `1px dashed ${grenaColor}`, color: grenaColor, textAlign: 'center', fontWeight: 700, fontSize: '0.85rem', textTransform: 'uppercase' }}>
                    Lançamento Avulso Ativado
                  </div>
                  <input autoFocus placeholder="Nome do Produto (ex: Porção Extra)" value={customName} onChange={e => setCustomName(e.target.value)} style={{ width: '100%', padding: '18px 20px', borderRadius: '12px', border: `1px solid ${borderLight}`, fontSize: '1rem', outline: 'none', background: '#fafafa' }} />
                  <input type="number" placeholder="Preço (R$ 0.00)" value={customPrice} onChange={e => setCustomPrice(e.target.value)} style={{ width: '100%', padding: '18px 20px', borderRadius: '12px', border: `1px solid ${borderLight}`, fontSize: '1.5rem', fontWeight: 700, outline: 'none', color: '#166534', textAlign: 'center', background: '#fafafa' }} />
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  {filteredProducts.length === 0 && <p style={{ textAlign: 'center', color: '#ccc', marginTop: '40px' }}>Nenhum produto encontrado.</p>}
                </div>
              )}
            </div>

            {/* BARRA DE AÇÃO COM O LÁPIS EMBAIXO */}
            <div style={{ padding: '20px', borderTop: `1px solid ${borderLight}`, background: '#fff' }}>
                <div style={{ display: 'flex', gap: '10px', height: '55px' }}>
                    ...
                    {/* O LÁPIS ESTÁ AQUI AGORA */}
                    <button 
                        onClick={() => setIsCustomMode(!isCustomMode)} 
                        style={{ width: '55px', borderRadius: '12px', border: `1px solid ${isCustomMode ? grenaColor : borderLight}`, background: isCustomMode ? '#fff1f2' : '#fff', color: isCustomMode ? grenaColor : luxuryBlack, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
                        title="Modo Avulso"
                    >
                    ...
                    <button 
                        ...
                        disabled={!isCustomMode && !selectedProductId}
                        style={{ flex: 1, background: (!isCustomMode && !selectedProductId) ? '#f5f5f5' : luxuryBlack, color: (!isCustomMode && !selectedProductId) ? '#ccc' : '#fff', border: 'none', borderRadius: '12px', fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '1px', transition: 'all 0.2s' }}
                    >
                        {isCustomMode ? 'Lançar' : 'Lançar Item'}
                    </button>
                </div>
            </div>
          </div>
        ...
        {/* CONTEÚDO COMANDA (RESUMO E PAGAMENTO) */}
        {activeTab === 'comanda' && !isPaymentStep && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#fafafa' }}>
            <div className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
              {items.length === 0 ? (
                <p style={{ textAlign: 'center', color: textMuted, marginTop: '40px' }}>Nenhum item na mesa.</p>
              ) : (
                  ...
                  {items.map(item => (
                    <div key={item.id} style={{ padding: '15px 20px', borderBottom: `1px solid ${borderLight}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        <span style={{ background: '#f0f0f0', padding: '4px 8px', borderRadius: '6px', fontWeight: 800, fontSize: '0.8rem' }}>{item.quantity}x</span>
                        <span style={{ fontWeight: 600, fontSize: '0.95rem' }}>{item.product_name_snapshot}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        <span style={{ fontWeight: 700 }}>R$ {(item.product_price_snapshot * item.quantity).toFixed(2)}</span>
                        {userRole === 'admin' && <button onClick={() => handleRemoveItem(item.id)} style={{ border: 'none', background: 'none', color: '#ef4444', cursor: 'pointer' }}><IconTrash /></button>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ padding: '25px', background: '#fff', borderTop: `1px solid ${borderLight}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px', fontSize: '1.2rem', fontWeight: 700 }}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                <button 
                    ...
                    style={{ flex: 1.5, padding: '15px', borderRadius: '12px', border: 'none', background: '#16a34a', color: '#fff', fontWeight: 700, cursor: 'pointer', textTransform: 'uppercase' }}
                >
                    Receber Pagamento
                </button>
              </div>
        ...
        {isPaymentStep && (
            <div className="slide-up" style={{ flex: 1, padding: '30px 20px', textAlign: 'center' }}>
                <h3 style={{ margin: '0 0 20px', fontSize: '1.2rem', color: luxuryBlack, fontWeight: 600, textAlign: 'center' }}>Qual o valor recebido agora?</h3>
